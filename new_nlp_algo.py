...
train_data, test_data, train_labels, test_labels = train_test_split(train['text'], train['label'], test_size=0.2, random_state=42)
...
def detect_emotion(message):
    ...
    Args:
        message (String): A message/messages a user says.
    ...
    for i in end_punctuation:
        input_message = message.replace(i, " ")
    
    input_message = input_message.split()
    
    input_data = tfidf_vectorizer.transform(input_message)
